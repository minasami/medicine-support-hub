...
async function supabaseRequest(path, init = {}) { const { url, key } = supabaseConfig(); const response = await fetch(`${url}${path}`, { ...init, headers: { apikey: key, Authorization: `Bearer ${key}`, "Content-Type": "application/json", ...(init.headers || {}) }, signal: AbortSignal.timeout(10000) }); if (!response.ok) throw new Error(`Supabase request failed: HTTP ${response.status}`); return response.json(); }
