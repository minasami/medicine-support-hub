...
function conciseDescription(product) {
  ...
  const facts = [
    ...
    product.category,
    product.route,
    product.price != null ? `highest observed price candidate ${Number(product.price).toLocaleString("en-US")} ${product.price_currency || "EGP"}` : null,
    Number(product.distinct_price_count || 0) > 1 ? `${product.distinct_price_count} observed price points` : null,
  ].filter(Boolean);
  const description = `${name}${facts.length ? ` — ${facts.join(", ")}` : ""}. Merged medicine record with source-backed price history.`;
...
function injectMeta(html, product, canonicalUrl, noindex = false) {
  ...
  const title = `${primaryName} | Price History and Medicine Information | Medicine Support Hub`;
    ...
    "@graph": [
      {
        ...
        ...(product.price != null ? { offers: { "@type": "Offer", url: canonicalUrl, price: String(product.price), priceCurrency: product.price_currency || "EGP" } } : {}),
        ...(product.source_url ? { isBasedOn: [product.source_url] } : {}),
      },
      {
        "@type": "BreadcrumbList",
        itemListElement: [
          { "@type": "ListItem", position: 1, name: "Medicine Support Hub", item: `${baseUrl}/` },
          { "@type": "ListItem", position: 2, name: "Medicine Search", item: `${baseUrl}/medicines` },
...
function requestHeader(request, name) {
  ...
  return Array.isArray(value) ? value.join(", ") : value ? String(value) : null;
}
...
async function fetchIndexHtml(request) {
  ...
  const response = await fetch(`${protocol}://${host}/index.html`, { headers, redirect: "follow", signal: AbortSignal.timeout(10000) });
  if (!response.ok) throw new Error(`Could not load application shell: HTTP ${response.status}`);
...
const canonicalFields = [
  "canonical_id", "name_en", "name_ar", "scientific_name", "manufacturer", "drug_class", "route", "category",
  "image_url", "egyptdwa_source_url", "barcode", "code", "current_price_egp", "price_currency",
  "distinct_price_count", "source_count", "has_verified_dataset",
].join(",");

function adaptCanonical(row) {
  return {
    id: row.canonical_id,
    name_en: row.name_en,
    name_ar: row.name_ar,
    ...
    category: row.drug_class || row.category,
    route: row.route,
    image_url: row.image_url,
    source_url: row.egyptdwa_source_url,
    barcode: row.barcode,
    code: row.code,
    price: row.current_price_egp,
    price_currency: row.price_currency || "EGP",
    distinct_price_count: row.distinct_price_count,
    source_count: row.source_count,
    has_verified_dataset: row.has_verified_dataset,
  };
}

async function canonicalById(id) {
  const rows = await supabaseRequest(`/rest/v1/medicine_canonical_products_v1?select=${canonicalFields}&canonical_id=eq.${id}&limit=1`);
  return rows[0] ? adaptCanonical(rows[0]) : null;
}

async function mapMedicines2Id(id) {
  const rows = await supabaseRequest(`/rest/v1/medicine_catalog_id_map_v1?select=canonical_id&source_system=eq.medicines2&source_record_key=eq.${id}&limit=1`);
...
async function loadProduct(id, legacy) {
  if (!legacy) {
    ...
    if (direct) return { product: direct, noindex: false };
    const canonicalId = await mapMedicines2Id(id);
    return canonicalId ? { product: await canonicalById(canonicalId), noindex: false } : { product: null, noindex: false };
  }

  const mappedLegacy = await supabaseRequest("/rest/v1/rpc/resolve_legacy_medicine_catalog", {
    method: "POST",
