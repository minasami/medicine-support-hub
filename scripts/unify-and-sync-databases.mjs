...
// Helper: Normalize keys for deduplication
...
// 1. Process Egyptian medicines.json (25,070 Ground Truth Medicines)
  ...
  for (const item of items) {
    ...
    if (!unifiedMap.has(key)) {
      unifiedMap.set(key, {
        ...
        category: drugClass,
        route: route,
        dosage_form: route.includes('.') ? route.split('.')[1] : route,
...
// 2. Process medicines2.csv (86,107 Barcode & Detailed Price Items)
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (unifiedMap.has(key)) {
      ...
      unifiedMap.set(key, {
        ...
        raw_manufacturer: 'Egyptian Pharmaceutical Industry',
        drug_class: 'Therapeutic Product',
...
// 3. Guarantee 100% Inclusion of Soul Pharma 5 Ground Truth Products
...
for (const sp of soulProducts) {
  const idx = unifiedArray.findIndex(m => m.canonical_id === sp.canonical_id || (m.name_en && m.name_en.toUpperCase() === sp.name_en));
  if (idx >= 0) {
...
console.log(`✅ Central Unified Database compiled with ${unifiedArray.length} total deduplicated medicines.`);

// 4. Save to public and src asset paths
...
if (fs.existsSync(path.dirname(srcPath))) {
  // Sliced inlined fallback for fast bundler chunking
  const srcPayload = {
    version: "4.0.0",
    last_updated: new Date().toISOString(),
    total_count: unifiedArray.length,
