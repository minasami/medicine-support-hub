...
function normalizePublicSiteUrl(value) {
  ...
  if (parsed.username || parsed.password || parsed.search || parsed.hash) {
    throw new Error(
      "VITE_PUBLIC_SITE_URL must be a clean public origin without credentials, query, or hash.",
    );
  }
...
async function rewritePublicOrigin(directory, publicSiteUrl) {
  ...
  for (const entry of await fs.readdir(directory, { withFileTypes: true })) {
    ...
    await fs.writeFile(
      entryPath,
      original.replaceAll(legacyPublicOrigin, publicSiteUrl),
      "utf8",
    );
    rewrittenFiles += 1;
  }
...
for (const route of [...routes].sort()) {
  ...
  if (
    ...
    throw new Error(
      `Unsafe route discovered while preparing Appwrite output: ${route}`,
    );
  }
...
const integrationStatusDirectory = path.join(
  outputDirectory,
  "api",
  "admin-integrations",
);
...
await fs.writeFile(
  ...
  JSON.stringify({
    google_document_ai: {
      configured: false,
      provider: "unavailable_on_appwrite_static_preview",
    },
    firecrawl: {
      configured: false,
