...
const staticRoutes = [
  ...
  "/vision", "/platform", "/solutions", "/security", "/research", "/contact", "/brand", "/disclosures", "/ngo",
...
function entityPath(type, slug) { const prefix = type === "company" ? "companies" : type === "generic" ? "generics" : "diseases"; return `/${prefix}/${encodeURIComponent(slug)}`; }
async function fetchJsonWithRetry(url, headers, attempts = 3) { let lastError; for (let attempt = 1; attempt <= attempts; attempt += 1) { try { const response = await fetch(url, { headers, signal: AbortSignal.timeout(20000) }); if (!response.ok) throw new Error(`HTTP ${response.status}: ${await response.text()}`); return response.json(); } catch (error) { lastError = error; if (attempt < attempts) await new Promise((resolve) => setTimeout(resolve, attempt * 750)); } } throw lastError; }
function supabaseContext() { const url = process.env.VITE_SUPABASE_URL?.replace(/\/+$/, ""); const key = process.env.VITE_SUPABASE_PUBLISHABLE_KEY; if (!url || !key) return null; return { url, headers: { apikey: key, Authorization: `Bearer ${key}`, Accept: "application/json" } }; }
async function fetchCatalogIds(context) { if (!context) return []; const ids = []; let lastId = 0; while (true) { const query = new URLSearchParams({ select: "canonical_id", order: "canonical_id.asc", limit: String(requestPageSize) }); if (lastId > 0) query.set("canonical_id", `gt.${lastId}`); const rows = await fetchJsonWithRetry(`${context.url}/rest/v1/medicine_encyclopedia_products_v2?${query.toString()}`, context.headers); if (!Array.isArray(rows) || rows.length === 0) break; for (const row of rows) { const id = Number(row?.canonical_id); if (Number.isSafeInteger(id) && id > 0) ids.push(id); } const nextLastId = Number(rows.at(-1)?.canonical_id); if (!Number.isSafeInteger(nextLastId) || nextLastId <= lastId) throw new Error("Canonical sitemap pagination did not advance."); lastId = nextLastId; if (rows.length < requestPageSize) break; } return [...new Set(ids)].sort((a, b) => a - b); }

async function fetchEntityDirectory(context) {
  if (!context) return { generatedAt: new Date().toISOString(), entities: [], sellers: [] };
  const companyQuery = new URLSearchParams({ select: "company_name,company_slug,product_count,generic_count,drug_class_count,min_price_egp,max_price_egp,leading_generics,leading_classes,leading_routes,generated_at", order: "product_count.desc,company_name.asc", limit: "5000" });
  ...
  const [companies, facets, officialCompanies, sellers] = await Promise.all([
    fetchJsonWithRetry(`${context.url}/rest/v1/medicine_manufacturer_profiles_generated?${companyQuery.toString()}`, context.headers),
  ...
  for (const row of Array.isArray(companies) ? companies : []) { const slug = String(row.company_slug || "").trim(); const name = String(row.company_name || "").trim(); if (!slug || !name) continue; companyEntities.set(slug, { type: "company", slug, name, records: Number(row.product_count || 0), activeRecords: Number(row.product_count || 0), prescriptionRecords: 0, genericCount: Number(row.generic_count || 0), diseaseCount: Number(row.drug_class_count || 0), origin: null, minPrice: row.min_price_egp == null ? null : Number(row.min_price_egp), maxPrice: row.max_price_egp == null ? null : Number(row.max_price_egp), leadingGenerics: row.leading_generics || [], leadingClasses: row.leading_classes || [], leadingRoutes: row.leading_routes || [], generatedAt: row.generated_at || null }); }
...
async function main() {
  await rm(outputDir, { recursive: true, force: true }); await mkdir(outputDir, { recursive: true }); const context = supabaseContext(); if (!context) console.warn("SEO generation: Supabase environment variables unavailable; publishing static routes only."); const files = ["static.xml"]; await writeFile(path.join(outputDir, "static.xml"), urlSet(staticRoutes.map((route) => `${baseUrl}${route}`)), "utf8"); const [productIds, directory] = await Promise.all([fetchCatalogIds(context), fetchEntityDirectory(context)]); for (let offset = 0; offset < productIds.length; offset += sitemapPageSize) { const filename = `catalog-${Math.floor(offset / sitemapPageSize) + 1}.xml`; await writeFile(path.join(outputDir, filename), urlSet(productIds.slice(offset, offset + sitemapPageSize).map((id) => `${baseUrl}/catalog/${id}`)), "utf8"); files.push(filename); } await writeFile(entityDirectoryPath, `${JSON.stringify(directory, null, 2)}\n`, "utf8"); const entityUrls = directory.entities.map((entity) => `${baseUrl}${entityPath(entity.type, entity.slug)}`); if (entityUrls.length) { await writeFile(path.join(outputDir, "entities.xml"), urlSet(entityUrls), "utf8"); files.push("entities.xml"); } const sellerUrls = directory.sellers.map((seller) => `${baseUrl}/marketplace/sellers/${encodeURIComponent(seller.seller_slug)}`); if (sellerUrls.length) { await writeFile(path.join(outputDir, "marketplace-sellers.xml"), urlSet(sellerUrls), "utf8"); files.push("marketplace-sellers.xml"); } await writeFile(sitemapIndexPath, sitemapIndex(files), "utf8"); console.log(`SEO generation: ${staticRoutes.length} static URLs, ${productIds.length} encyclopedia URLs, ${entityUrls.length} entity URLs, and ${sellerUrls.length} verified seller URLs across ${files.length} sitemap files.`);
}
await main();
