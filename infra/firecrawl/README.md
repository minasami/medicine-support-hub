...
Use the official Firecrawl repository and its `SELF_HOST.md` or Kubernetes/Helm examples. Pin the upstream commit or release, preserve the upstream AGPL-3.0 licensing obligations, use strong database and queue-admin credentials, keep Redis and PostgreSQL private, and expose the API only through HTTPS.
...
The daily Vercel cron runs only when all of the following are true:
