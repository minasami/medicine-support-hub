...
2. Run the Platform Health workflow manually or wait for the scheduled run, then confirm the `production-smoke-test` job passes.
...
6. Re-check the Notion operating page for stale deployment/security status.
7. Review PR #36 first because its build and typecheck are passing; keep draft PRs #25/#26 out of the merge path until rebased and revalidated.
