create table if not exists public.entity_favorites (
  ...
  entity_type text not null check (entity_type in ('medicine','company')), entity_key text not null,
  created_at timestamptz not null default now(), unique(user_id,entity_type,entity_key)
);
...
create table if not exists public.entity_comments (
  id uuid primary key default gen_random_uuid(), entity_type text not null check (entity_type in ('medicine','company')),
  entity_key text not null, parent_id uuid references public.entity_comments(id) on delete cascade,
  user_id uuid not null references auth.users(id) on delete cascade, body text not null check (char_length(btrim(body)) between 2 and 2000),
  is_anonymous boolean not null default false, status text not null default 'pending' check (status in ('pending','approved','rejected','hidden')),
...
create table if not exists public.entity_public_comments (
  id uuid primary key, entity_type text not null, entity_key text not null, parent_id uuid,
  body text not null, author_name text not null, created_at timestamptz not null
);
...
create table if not exists public.entity_social_stats (
  entity_type text not null check (entity_type in ('medicine','company')), entity_key text not null,
  favorite_count bigint not null default 0, comment_count bigint not null default 0,
  updated_at timestamptz not null default now(), primary key(entity_type,entity_key)
);
...
begin
  v_type:=coalesce(new.entity_type,old.entity_type); v_key:=coalesce(new.entity_key,old.entity_key);
  if tg_table_name='entity_comments' then
    if tg_op='DELETE' or new.status<>'approved' then delete from public.entity_public_comments where id=old.id;
    else
  ...
  values(v_type,v_key,(select count(*) from public.entity_favorites f where f.entity_type=v_type and f.entity_key=v_key),(select count(*) from public.entity_comments c where c.entity_type=v_type and c.entity_key=v_key and c.status='approved'),now())
