create table if not exists public.push_subscriptions (
  ...
  endpoint text not null unique, p256dh text not null, auth_key text not null, user_agent text, locale text,
  topics text[] not null default array['platform_updates']::text[], is_enabled boolean not null default true,
  failure_count integer not null default 0 check (failure_count >= 0), last_seen_at timestamptz not null default now(),
...
create table if not exists public.notification_preferences (
  ...
  platform_updates boolean not null default true, medicine_updates boolean not null default true,
  company_updates boolean not null default true, marketplace_updates boolean not null default true,
  learning_updates boolean not null default true, favorite_updates boolean not null default true,
  quiet_hours_start time, quiet_hours_end time, locale text,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);

create table if not exists public.notification_campaigns (
  id uuid primary key default gen_random_uuid(), title text not null check (char_length(btrim(title)) between 2 and 120),
  ...
  target_url text, icon_url text, image_url text, data jsonb not null default '{}',
  ...
  attempted_count integer not null default 0, delivered_count integer not null default 0, failed_count integer not null default 0,
...
create table if not exists public.user_notifications (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade,
  campaign_id uuid references public.notification_campaigns(id) on delete set null, title text not null, body text not null,
  target_url text, notification_topic text not null default 'platform_updates', entity_type text, entity_key text,
...
create table if not exists public.notification_deliveries (
  ...
  user_id uuid references auth.users(id) on delete set null, subscription_id uuid references public.push_subscriptions(id) on delete set null,
  status text not null default 'queued' check (status in ('queued','sent','failed','expired','opened')),
  provider_status integer, failure_reason text, sent_at timestamptz, opened_at timestamptz, created_at timestamptz not null default now()
);
