...
with company_active as (
  ...
  select base.canonical_id, base.canonical_key,
    coalesce(base.name_en,company.name_en) name_en,
    coalesce(base.name_ar,company.name_ar) name_ar,
    coalesce(base.scientific_name,company.scientific_name) scientific_name,
    coalesce(base.manufacturer,company.manufacturer) manufacturer,
    coalesce(base.drug_class,company.drug_class) drug_class,
    coalesce(base.route,company.route) route,
    coalesce(base.category,company.category) category,
    coalesce(base.image_url,company.image_url) image_url,
    base.egyptdwa_source_url,
    coalesce(base.barcode,company.barcode) barcode,
    coalesce(base.code,company.product_code) code,
  ...
  select company.canonical_id, company.canonical_key, company.name_en, company.name_ar,
    ...
    coalesce(prices.distinct_count,0) distinct_price_count,
