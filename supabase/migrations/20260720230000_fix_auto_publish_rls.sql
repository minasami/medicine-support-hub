-- 20260720230000_fix_auto_publish_rls.sql
-- Change auto publish trigger to AFTER INSERT to avoid violating the strict RLS INSERT policies.
...
begin
  ...
  if new.submitter_kind = 'company_representative' and new.status = 'submitted' then
    ...
    if found and profile_row.verification_status = 'verified' then
      ...
      -- Update the submission status asynchronously since it's already inserted.
      -- This avoids breaking the strict RLS INSERT policy which expects status='submitted'.
      update public.medicine_catalog_submissions 
      set status = 'published',
          reviewed_by = new.submitted_by,
          reviewed_at = now(),
          review_notes = 'Auto-published for verified company representative.'
      where id = new.id;
...
after insert on public.medicine_catalog_submissions
for each row
