-- Final reproducible medicine search v4 release.
-- Builds the connected search document once, indexes it, and separates exact/full-text/fuzzy plans.

drop function if exists public.search_medicine_encyclopedia_v4(text,text,text,text,text,text,text,numeric,numeric,boolean,boolean,boolean,boolean,integer,text,text,integer,integer);
drop function if exists public.refresh_medicine_search_index_v1();
drop materialized view if exists private.medicine_search_index_v1;
...
create index medicine_search_index_v1_name_en_lower_idx on private.medicine_search_index_v1(lower(name_en)) where name_en is not null;
create index medicine_search_index_v1_name_ar_lower_idx on private.medicine_search_index_v1(lower(name_ar)) where name_ar is not null;
