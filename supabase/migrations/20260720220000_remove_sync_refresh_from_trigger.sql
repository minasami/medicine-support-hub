-- 20260720220000_remove_sync_refresh_from_trigger.sql
-- Remove the synchronous TRUNCATE and MATERIALIZED VIEW refreshes from the row-level trigger 
-- to prevent transaction locks and "cannot TRUNCATE... active queries" errors.
...
begin
  ...
  if new.submitter_kind = 'company_representative' and new.status = 'submitted' then
    ...
    if found and profile_row.verification_status = 'verified' then
      -- Auto approve the submission row
      ...
      -- Note: Removed synchronous refresh of materialized views here to prevent
      -- "cannot TRUNCATE... active queries" errors inside row-level triggers.
      -- The main encyclopedia view is updated instantly anyway.
    end if;
  end if;
