...
as $$
  select coalesce(
    nullif(trim(both '-' from regexp_replace(lower(coalesce(value,'')),'[^a-z0-9]+','-','g')),''),
    'manufacturer-'||substr(md5(coalesce(value,'')),1,12)
  );
$$;
...
create table if not exists public.medicine_manufacturer_profiles_generated (
  ...
  drug_class_count integer not null default 0,
  route_count integer not null default 0,
  products_with_images integer not null default 0,
  products_with_price_history integer not null default 0,
  products_with_marketplace_offers integer not null default 0,
  min_price_egp numeric,
  max_price_egp numeric,
  leading_generics text[] not null default '{}',
  leading_classes text[] not null default '{}',
  leading_routes text[] not null default '{}',
  portfolio_sample_ids bigint[] not null default '{}',
  portfolio_sample_names text[] not null default '{}',
  source_name text not null default 'Canonical medicine encyclopedia',
...
begin
  ...
  insert into public.medicine_manufacturer_profiles_generated(
    ...
    products_with_images,products_with_price_history,products_with_marketplace_offers,min_price_egp,max_price_egp,
    leading_generics,leading_classes,leading_routes,portfolio_sample_ids,portfolio_sample_names,generated_at
  )
  select
    public.medicine_manufacturer_slug(manufacturer) company_slug,
    (array_agg(manufacturer order by completeness_score desc,canonical_id))[1] company_name,
    count(*)::integer product_count,
    count(distinct nullif(btrim(scientific_name),''))::integer generic_count,
    count(distinct nullif(btrim(drug_class),''))::integer drug_class_count,
    count(distinct nullif(btrim(route),''))::integer route_count,
    count(*) filter(where nullif(btrim(image_url),'') is not null)::integer products_with_images,
    count(*) filter(where has_price_history)::integer products_with_price_history,
    count(*) filter(where marketplace_offer_count>0)::integer products_with_marketplace_offers,
    ...
    (array_agg(coalesce(name_en,name_ar,'Medicine #'||canonical_id::text) order by completeness_score desc,source_count desc,canonical_id))[1:12],
    now()
...
returns table(
  canonical_id bigint,name_en text,name_ar text,scientific_name text,manufacturer text,drug_class text,route text,category text,
  image_url text,current_price_egp numeric,price_currency text,has_price_history boolean,source_count integer,
...
as $$
  with filtered as (
    select p.*
    from private.medicine_search_index_v1 p
  ...
  select f.canonical_id,f.name_en,f.name_ar,f.scientific_name,f.manufacturer,f.drug_class,f.route,f.category,
    f.image_url,f.current_price_egp,f.price_currency,f.has_price_history,f.source_count,f.marketplace_offer_count,
