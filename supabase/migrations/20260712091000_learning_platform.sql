-- Public learning catalog with private learner progress.
...
create trigger learning_courses_touch_updated_at before update on public.learning_courses for each row execute function private.touch_updated_at();
create trigger learning_lessons_touch_updated_at before update on public.learning_lessons for each row execute function private.touch_updated_at();
create trigger learning_enrollments_touch_updated_at before update on public.learning_enrollments for each row execute function private.touch_updated_at();
...
values
('patient-health-journey','Your connected healthcare journey','رحلتك الصحية المترابطة','Learn how to claim your health record, review consent requests, follow prescriptions and diagnostic orders, receive results, and track insurance decisions.','تعلّم كيفية ربط سجلك الصحي ومراجعة طلبات الموافقة ومتابعة الوصفات وطلبات الفحوص والنتائج وقرارات التأمين.',array['patient'],array['patient'],array['Claim and protect your patient record','Review care-team access','Follow orders, results, medicines, and approvals'],'foundation',true,10),
('physician-connected-care','Physician connected-care workflow','مسار الرعاية المترابطة للطبيب','Create or find a consented patient, document an encounter, prescribe structured medicines, order diagnostics, and request payer authorization.','أنشئ أو ابحث عن مريض بموافقة موثقة وسجّل الزيارة والوصفة وطلبات الفحوص وموافقة جهة التأمين.',array['physician'],array['clinic','hospital'],array['Use privacy-safe patient matching','Create structured prescriptions','Route laboratory, imaging, pharmacy, and insurance work'],'foundation',true,20),
('pharmacy-clinical-fulfillment','Pharmacy clinical fulfillment','تنفيذ الوصفات بالصيدلية','Receive routed prescription items, verify the patient and order, document dispensing, and preserve batch and audit evidence.','استقبل بنود الوصفات المحالة وتحقق من المريض والطلب وسجّل الصرف وأدلة التشغيلة والمراجعة.',array['pharmacist','pharmacy_assistant'],array['commercial_pharmacy','pharmacy_partner'],array['Receive routed medication orders','Record safe dispensing status','Connect fulfillment to the patient timeline'],'foundation',true,30),
...
('payer-authorization-workflow','Insurance authorization workflow','مسار موافقات التأمين','Review coverage and prior-authorization requests, request missing information, and return attributable decisions to the care journey.','راجع التغطية وطلبات الموافقة المسبقة واطلب المعلومات الناقصة وأعد القرارات المنسوبة إلى رحلة الرعاية.',array['insurance_reviewer'],array['insurance_company','payer'],array['Review requested services safely','Document approval decisions','Connect decisions to prescriptions and diagnostic orders'],'foundation',true,50),
('institution-platform-onboarding','Institution platform onboarding','تهيئة المؤسسات على المنصة','Configure organization membership, clinical access, operating roles, training assignments, governance, and launch readiness.','اضبط عضوية المؤسسة والوصول السريري والأدوار التشغيلية والتدريب والحوكمة والاستعداد للإطلاق.',array['org_admin','program_manager'],array['clinic','hospital','laboratory','radiology_center','insurance_company','ngo','pharmacy'],array['Configure a governed workspace','Assign least-privilege roles','Prepare staff and launch controls'],'administrator',true,60),
...
on conflict(slug) do update set title_en=excluded.title_en,title_ar=excluded.title_ar,summary_en=excluded.summary_en,summary_ar=excluded.summary_ar,
  audience_roles=excluded.audience_roles,audience_organization_types=excluded.audience_organization_types,learning_outcomes=excluded.learning_outcomes,
  level=excluded.level,is_published=excluded.is_published,sort_order=excluded.sort_order;
...
from public.learning_courses c
join (values
('patient-health-journey','claim-and-consent','Claim your record and control access','ربط السجل والتحكم في الوصول','Use the one-time claim code, verify your profile, and approve or deny care-team access.','استخدم رمز الربط لمرة واحدة وتحقق من ملفك ووافق أو ارفض وصول فريق الرعاية.',7,1,jsonb_build_object('steps',array['Sign in or create an account','Enter the claim code','Review pending access requests','Grant only the needed scopes'])),
('patient-health-journey','follow-the-journey','Follow your care timeline','متابعة خط الرعاية','Understand encounters, prescriptions, diagnostic orders, results, dispensing, and payer decisions.','افهم الزيارات والوصفات وطلبات الفحوص والنتائج والصرف وقرارات التأمين.',8,2,jsonb_build_object('steps',array['Open Health Record','Review active orders','Read final results','Check authorization and dispensing status'])),
('physician-connected-care','patient-match','Find or create the right patient','العثور على المريض الصحيح أو إنشاؤه','Search only accessible patients by name or perform an audited exact identity match.','ابحث فقط داخل المرضى المصرح بهم بالاسم أو نفّذ مطابقة هوية دقيقة ومسجلة.',9,1,jsonb_build_object('steps',array['Choose your organization','Search accessible patients by name','Use exact identity match when needed','Request consent or create an invited record'])),
...
('pharmacy-clinical-fulfillment','dispense-safely','Receive and dispense safely','استلام وصرف آمن','Work only from routed prescription items and record dispensing outcomes without changing the prescriber order.','اعمل من بنود الوصفات المحالة وسجّل نتائج الصرف دون تغيير أمر الطبيب.',10,1,jsonb_build_object('steps',array['Verify routed item','Confirm medicine and quantity','Check prescription requirements','Record partial or complete dispensing'])),
...
on conflict(course_id,lesson_slug) do update set title_en=excluded.title_en,title_ar=excluded.title_ar,summary_en=excluded.summary_en,
  summary_ar=excluded.summary_ar,duration_minutes=excluded.duration_minutes,lesson_order=excluded.lesson_order,content=excluded.content,is_published=true;
