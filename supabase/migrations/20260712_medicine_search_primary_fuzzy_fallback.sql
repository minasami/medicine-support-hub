-- Use indexed full-text and exact matching first.
-- Trigram fuzzy matching runs only when the primary query finds no products.
...
begin
  ...
  with terms as (
    ...
    where q=''
      ...
      or lower(coalesce(indexed.name_en,''))=q_normalized
      or lower(coalesce(indexed.name_ar,''))=q_normalized
      or indexed.search_vector @@ text_query
  ), primary_state as (
    select exists(select 1 from primary_candidates where q<>'') has_primary
  ), base_candidates as (
    select primary_candidates.* from primary_candidates
    union all
    select indexed.*
    from private.medicine_search_index_v1 indexed
    cross join primary_state
    where q<>'' and not primary_state.has_primary
      and (
        indexed.name_en % q
