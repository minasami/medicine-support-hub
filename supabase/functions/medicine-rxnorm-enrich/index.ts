...
type RxConcept = { rxcui?: string; name?: string; synonym?: string; tty?: string; language?: string; suppress?: string; umlscui?: string };
type RxGroup = { tty?: string; conceptProperties?: RxConcept[] };
...
function normalizeSearchName(medicine: Medicine) {
  return (medicine.name_en || medicine.name_ar || "").replace(/\b\d+\s?(mg|mcg|g|gm|ml|iu|%)\b/gi, "").replace(/\b\d+\b/g, "").replace(/\b(f\.?c\.?tabs?|tabs?|caps?|cream|gel|syrup|suspension|vial|ampoule|drops?)\b/gi, "").replace(/\s+/g, " ").trim();
}
...
function chooseIngredient(groups: RxGroup[]) {
  const concepts = groups.flatMap(group => group.conceptProperties || []);
  return concepts.find(item => item.tty === "IN") || concepts.find(item => item.tty === "MIN") || concepts.find(item => item.tty === "PIN") || concepts[0] || null;
}
...
Deno.serve(async (req: Request) => {
  ...
  const sourceUrl = `https://rxnav.nlm.nih.gov/REST/drugs.json?name=${encodeURIComponent(query)}`;
  const rxRes = await fetch(sourceUrl, { headers: { Accept: "application/json" } });
  const rxJson = await rxRes.json().catch(() => ({}));
  if (!rxRes.ok) return send({ error: "RxNorm request failed", source_url: sourceUrl }, 502);

  const groups = (rxJson?.drugGroup?.conceptGroup || []) as RxGroup[];
  const ingredient = chooseIngredient(groups);
  const allConcepts = groups.flatMap(group => (group.conceptProperties || []).map(item => ({ ...item, group_tty: group.tty })));
  if (!ingredient) return send({ medicine, query, source_url: sourceUrl, matches: [], inserted: null, message: "No RxNorm matches found" });

  const enrichment = {
    medicine_id: medicine.id,
    manufacturer: null,
    active_ingredient: ingredient.name || ingredient.synonym || null,
    atc_code: null,
    barcode: null,
    medicine_family: ingredient.tty || null,
    medicine_genre: "RxNorm normalized drug concept",
    route: null,
    source_name: "RxNorm RxNav API",
    source_url: sourceUrl,
    source_type: "trusted_reference",
    confidence: "needs_review",
    notes: `RxNorm RXCUI: ${ingredient.rxcui || "unknown"}. TTY: ${ingredient.tty || "unknown"}. Search query: ${query}.`,
  };
  ...
  return send({ medicine, query, source_url: sourceUrl, inserted: insertedRows[0] || null, matches: allConcepts.slice(0, 25) });
});
