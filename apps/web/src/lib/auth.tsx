...
type StaffSession = { access_token: string; refresh_token?: string; user?: { id: string; email?: string } };
type ProfileRow = { id: string; full_name: string | null; role: string; is_active: boolean };
...
function mapRole(role: string): NonNullable<UserRole> | null {
  ...
  if (["admin", "platform_admin", "super_admin"].includes(r)) return "PLATFORM_ADMIN";
...
async function hydrate(session: StaffSession): Promise<StaffSession> {
  ...
  const response = await fetch(`${url}/auth/v1/user`, { headers: { apikey: key, Authorization: `Bearer ${session.access_token}` } });
...
async function profileFor(session: StaffSession): Promise<ProfileRow> {
  ...
  const response = await fetch(`${url}/rest/v1/profiles?select=id,full_name,role,is_active&id=eq.${current.user?.id}&limit=1`, { headers: { apikey: key, Authorization: `Bearer ${current.access_token}` } });
  const rows = await response.json();
  if (!response.ok) throw new Error(rows?.message || "Could not read user profile.");
  if (!rows?.length) throw new Error("No platform profile exists for this account.");
  return rows[0];
}
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const [session, setSession] = useState<StaffSession | null>(() => readOAuthSession() ?? loadSession());

  async function applySession(next: StaffSession | null) {
    if (!next) { saveSession(null); setSession(null); setUser(null); return; }
    const current = await hydrate(next);
    const profile = await profileFor(current);
    if (!profile.is_active) throw new Error("This platform account is inactive.");
    const role = mapRole(profile.role);
    if (!role) throw new Error("This account does not have a staff platform role.");
    saveSession(current);
    setSession(current);
    setUser({ id: 1, username: current.user?.email ?? profile.id, role, displayName: profile.full_name || current.user?.email || "Platform user", branchId: null });
  }

  useEffect(() => {
    if (!session?.access_token) { setLoading(false); return; }
    applySession(session).catch(() => { saveSession(null); setUser(null); }).finally(() => setLoading(false));
  }, []);
  ...
  const logout = async () => { saveSession(null); setSession(null); setUser(null); };

  return <AuthContext.Provider value={{ loading, login, loginWithGoogle, logout }}>{children}</AuthContext.Provider>;
}
