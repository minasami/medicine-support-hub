  ...
  return (
    <div className="container mx-auto max-w-6xl px-4 py-8">
        ...
        {stats.map((stat) => (
          <Card key={stat.label}>
            <CardContent className="flex items-center justify-between p-5">
              ...
              <div className="rounded-xl bg-emerald-50 p-3 text-emerald-700"><stat.icon className="h-5 w-5" /></div>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <div className="mb-8 grid gap-4 md:grid-cols-3">
        <Card><CardHeader><CardTitle>Budget remaining</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-emerald-700">{money(totals.remaining, currency)}</div><p className="text-sm text-muted-foreground">Total budget minus committed and spent amounts.</p></CardContent></Card>
        <Card><CardHeader><CardTitle>Spent</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{money(totals.spent, currency)}</div><p className="text-sm text-muted-foreground">Recorded spending from active budgets.</p></CardContent></Card>
        <Card><CardHeader><CardTitle>Total budget</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{money(totals.totalBudget, currency)}</div><p className="text-sm text-muted-foreground">All active NGO program budgets.</p></CardContent></Card>
      </div>

      <div className="mb-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {modules.map((module) => (
          <Card key={module.href} className="transition hover:-translate-y-0.5 hover:shadow-md">
            <CardHeader><CardTitle className="flex items-center gap-2"><module.icon className="h-5 w-5 text-emerald-700" /> {module.label}</CardTitle></CardHeader>
            <CardContent className="space-y-4"><p className="text-sm text-muted-foreground">{module.description}</p><Button asChild size="sm" variant="secondary"><Link href={module.href}>Open module</Link></Button></CardContent>
          </Card>
        ))}
      </div>
      ...
      <Card>
        ...
        <CardContent><div className="grid gap-3 md:grid-cols-3">{["Beneficiary intake", "Medicine request", "Medical review", "Cost and budget review", "Approval", "Fulfillment tracking"].map((step, index) => <div key={step} className="flex items-center gap-3 rounded-lg border p-3"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-emerald-100 text-sm font-bold text-emerald-700">{index + 1}</div><div className="font-medium">{step}</div></div>)}</div></CardContent>
      </Card>
    </div>
  );
}
