  ...
  async function load() {
    ...
    try {
      const [c, o, r, m] = await Promise.all([
        supabaseFetch<CommentRow[]>("/rest/v1/community_comments?select=*&status=eq.held&order=created_at.desc&limit=50").catch(() => []),
        supabaseFetch<Observation[]>("/rest/v1/community_safety_observations?select=*&status=eq.submitted&order=created_at.desc&limit=50").catch(() => []),
        supabaseFetch<Report[]>("/rest/v1/community_content_reports?select=*&status=eq.submitted&order=created_at.desc&limit=50").catch(() => []),
        supabaseFetch<CompanyMessage[]>("/rest/v1/company_messages?select=*&status=in.(sent,unread)&order=created_at.desc&limit=50").catch(() => []),
      ]);
      ...
      setError(cause instanceof Error ? cause.message : "Could not load moderation items.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main className="container mx-auto max-w-7xl px-4 py-8 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">Community Moderation &amp; Safety</h1>
      ...
      <div className="grid gap-6">
        <Card>
          <CardHeader>
            <CardTitle>Held Community Comments ({comments.length})</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {comments.map((row) => (
              <div key={row.id} className="rounded-xl border p-4 space-y-2">
                <div className="flex justify-between text-xs text-muted-foreground">
                  <span>{row.author_name} · {humanize(row.entity_type)} ({row.entity_key})</span>
                  <Badge variant="secondary">{row.status}</Badge>
                </div>
                <p className="text-sm">{row.body}</p>
              </div>
            ))}
            {comments.length === 0 && <p className="text-sm text-muted-foreground">No held comments pending moderation.</p>}
          </CardContent>
        </Card>
      </div>
    </main>
