...
export default function NgoDirectoryPage() {
  ...
  const categoryLabel = (category: string) => {
    const labels: Record<string, [string, string]> = {
      medicines: ["Medicines support", "دعم دوائي"],
      medical: ["Medical & surgery", "طبي وجراحي"],
      social: ["Social aid", "دعم اجتماعي"],
      economic: ["Economic empowerment", "تمكين اقتصادي"],
      educational: ["Educational", "توعوي وتعليمي"],
    };
    const pair = labels[category] || ["Support", "دعم"];
    return t(pair[0], pair[1]);
  };

  const cityLabel = (city: string) => {
    if (!isAr) return city;
    if (city === "Cairo") return "القاهرة";
    if (city === "Giza") return "الجيزة";
    if (city === "Aswan") return "أسوان";
    return city;
  };
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="bg-white p-6 rounded-2xl border shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0">
            <Button variant={selectedCategory === "all" ? "default" : "outline"} onClick={() => setSelectedCategory("all")} className="rounded-xl text-xs font-semibold whitespace-nowrap">
              {t("All Programs", "كل البرامج")}
            </Button>
            <Button variant={selectedCategory === "medicines" ? "default" : "outline"} onClick={() => setSelectedCategory("medicines")} className="rounded-xl text-xs font-semibold whitespace-nowrap">
              <Pill className="mr-1 h-3.5 w-3.5 text-emerald-500" />
              {t("Medicines", "دعم دوائي")}
            </Button>
            <Button variant={selectedCategory === "medical" ? "default" : "outline"} onClick={() => setSelectedCategory("medical")} className="rounded-xl text-xs font-semibold whitespace-nowrap">
              <Stethoscope className="mr-1 h-3.5 w-3.5 text-blue-500" />
              {t("Medical & Surgeries", "عمليات وطب")}
            </Button>
            <Button variant={selectedCategory === "social" ? "default" : "outline"} onClick={() => setSelectedCategory("social")} className="rounded-xl text-xs font-semibold whitespace-nowrap">
              <Heart className="mr-1 h-3.5 w-3.5 text-rose-500" />
              {t("Social Aid", "اجتماعي ورعاية")}
            </Button>
            <Button variant={selectedCategory === "economic" ? "default" : "outline"} onClick={() => setSelectedCategory("economic")} className="rounded-xl text-xs font-semibold whitespace-nowrap">
              ...
              <Card key={ngo.id} className="rounded-2xl border p-5 space-y-3 bg-white">
                <div className="flex items-center justify-between">
                  <Badge variant="outline" className="text-xs bg-emerald-50 text-emerald-700 border-emerald-200">
                    <ShieldCheck className="mr-1 h-3 w-3" /> {t("Verified NGO", "جمعية موثّقة")}
                  </Badge>
                ...
                <div className="pt-2 text-xs text-slate-500 border-t flex justify-between">
                  <span>📍 {cityLabel(ngo.city)}{isAr ? "، مصر" : `, ${ngo.country}`}</span>
                  <span className="font-semibold text-emerald-700">{ngo.activeProgramsCount} {t("Programs", "برامج")}</span>
                </div>
              </Card>
            ))}
          </div>
            ...
            <form onSubmit={handleApplySubmit} className="space-y-4 py-2">
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder={t("Type to search e.g. Ketomax, Controloc...", "اكتب للبحث مثل: كيتوماكس، كونترولوك...")}
