...
export default function WorkspacePage() {
  ...
  return <div className="container mx-auto max-w-7xl px-4 py-8">
    <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
      <div><Badge className="mb-3 bg-blue-100 text-blue-700 hover:bg-blue-100">Enterprise Workspace v1</Badge><h1 className="flex items-center gap-3 text-3xl font-bold"><Building2 className="h-8 w-8 text-blue-700" />{organization?.name ?? "Organization Workspace"}</h1><p className="mt-2 text-muted-foreground">{organization ? `${organization.city ?? ""}${organization.city && organization.country ? ", " : ""}${organization.country ?? ""} • Role: ${membership?.role}` : "Programs, beneficiaries, budgets, and organizational operations."}</p></div>
      <div className="flex gap-2"><Button asChild variant="outline"><Link href="/ngo/dashboard">NGO dashboard</Link></Button><Button variant="outline" onClick={load} disabled={loading}><RefreshCw className="mr-2 h-4 w-4" />Refresh</Button></div>
    </div>
    ...
    <div className="mb-8 grid gap-4 sm:grid-cols-3">
      <Card><CardContent className="flex items-center justify-between p-5"><div><div className="text-2xl font-bold">{programs.length}</div><div className="text-sm text-muted-foreground">Programs</div></div><Building2 className="h-6 w-6 text-blue-700" /></CardContent></Card>
      <Card><CardContent className="flex items-center justify-between p-5"><div><div className="text-2xl font-bold">{beneficiaries.length}</div><div className="text-sm text-muted-foreground">Beneficiaries</div></div><Users className="h-6 w-6 text-emerald-700" /></CardContent></Card>
      <Card><CardContent className="flex items-center justify-between p-5"><div><div className="text-2xl font-bold">{Math.round(totalBudget).toLocaleString()} {organization?.currency ?? "EGP"}</div><div className="text-sm text-muted-foreground">Program budgets</div></div><Wallet className="h-6 w-6 text-amber-700" /></CardContent></Card>
    </div>
    <div className="grid gap-6 xl:grid-cols-2">
      <Card><CardHeader><CardTitle>Create program</CardTitle></CardHeader><CardContent className="space-y-4"><div><Label>Name</Label><Input value={programDraft.name} onChange={e => setProgramDraft({ ...programDraft, name: e.target.value })} placeholder="Diabetes Medicine Support" /></div><div><Label>Description</Label><Textarea value={programDraft.description} onChange={e => setProgramDraft({ ...programDraft, description: e.target.value })} /></div><div className="grid grid-cols-2 gap-3"><div><Label>Budget</Label><Input type="number" value={programDraft.budget_amount} onChange={e => setProgramDraft({ ...programDraft, budget_amount: e.target.value })} /></div><div><Label>Currency</Label><Input value={programDraft.currency} onChange={e => setProgramDraft({ ...programDraft, currency: e.target.value.toUpperCase() })} /></div></div><Button onClick={createProgram} disabled={saving || !programDraft.name.trim()}><Plus className="mr-2 h-4 w-4" />Create program</Button></CardContent></Card>
      <Card><CardHeader><CardTitle>Add beneficiary</CardTitle></CardHeader><CardContent className="space-y-4"><div><Label>Full name</Label><Input value={beneficiaryDraft.full_name} onChange={e => setBeneficiaryDraft({ ...beneficiaryDraft, full_name: e.target.value })} /></div><div className="grid grid-cols-2 gap-3"><div><Label>Phone</Label><Input value={beneficiaryDraft.phone} onChange={e => setBeneficiaryDraft({ ...beneficiaryDraft, phone: e.target.value })} /></div><div><Label>City</Label><Input value={beneficiaryDraft.city} onChange={e => setBeneficiaryDraft({ ...beneficiaryDraft, city: e.target.value })} /></div></div><div><Label>Primary condition</Label><Input value={beneficiaryDraft.primary_condition} onChange={e => setBeneficiaryDraft({ ...beneficiaryDraft, primary_condition: e.target.value })} /></div><div><Label>Program</Label><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={beneficiaryDraft.program_id} onChange={e => setBeneficiaryDraft({ ...beneficiaryDraft, program_id: e.target.value })}><option value="">Unassigned</option>{programs.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div><Button onClick={createBeneficiary} disabled={saving || !beneficiaryDraft.full_name.trim()}><Plus className="mr-2 h-4 w-4" />Add beneficiary</Button></CardContent></Card>
    </div>
    <div className="mt-6 grid gap-6 xl:grid-cols-2">
      <Card><CardHeader><CardTitle>Programs</CardTitle></CardHeader><CardContent className="space-y-3">{programs.length === 0 ? <p className="text-sm text-muted-foreground">No programs yet.</p> : programs.map(p => <div key={p.id} className="rounded-lg border p-4"><div className="flex items-center justify-between gap-3"><div className="font-semibold">{p.name}</div><Badge variant="secondary">{p.status}</Badge></div><p className="mt-1 text-sm text-muted-foreground">{p.description || "No description"}</p><div className="mt-2 text-sm font-medium">{Number(p.budget_amount || 0).toLocaleString()} {p.currency}</div></div>)}</CardContent></Card>
      <Card><CardHeader><CardTitle>Beneficiaries</CardTitle></CardHeader><CardContent className="space-y-3">{beneficiaries.length === 0 ? <p className="text-sm text-muted-foreground">No beneficiaries yet.</p> : beneficiaries.map(b => <div key={b.id} className="rounded-lg border p-4"><div className="flex items-center justify-between gap-3"><div className="font-semibold">{b.full_name}</div><Badge variant="secondary">{b.risk_level}</Badge></div><p className="mt-1 text-sm text-muted-foreground">{[b.primary_condition, b.city, b.phone].filter(Boolean).join(" • ") || "No additional details"}</p></div>)}</CardContent></Card>
    </div>
  </div>;
}
