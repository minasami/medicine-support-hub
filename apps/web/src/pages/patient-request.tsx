...
export default function PatientRequestPage() {
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      ...
      try {
        const safe = encodeURIComponent(`*${search.replace(/[(),]/g, " ")}*`);
        const fields = "id,legacy_medicine_id,name_en,name_ar,dosage_form,strength,category,barcode,price,price_currency,code";
        const rows = await supabaseFetch<Medicine[]>(`/rest/v1/medicines_catalog?select=${fields}&limit=12&or=(name_en.ilike.${safe},name_ar.ilike.${safe},barcode.ilike.${safe},code.ilike.${safe})`);
        setResults(rows);
      } catch { setResults([]); }
    }, 250);
  ...
  return <div className="container mx-auto max-w-4xl px-4 py-8">
    ...
    <form onSubmit={submit} className="space-y-6">
      ...
      <Card><CardHeader><CardTitle>Products</CardTitle><CardDescription>Search by English name, Arabic name, barcode, or product code. Existing legacy medicine IDs are used only when a verified compatibility match exists.</CardDescription></CardHeader><CardContent className="space-y-4">
        <div className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input className="pl-10" value={query} onChange={e => setQuery(e.target.value)} placeholder="Search full product catalog" />{!!results.length && <div className="absolute z-20 mt-1 max-h-80 w-full overflow-auto rounded-md border bg-background shadow-lg">{results.map(med => <button key={med.id} type="button" className="w-full px-3 py-3 text-left hover:bg-muted" onClick={() => addMedicine(med)}><div className="font-medium">{med.name_en || med.name_ar}</div><div className="text-xs text-muted-foreground">{med.name_ar} {med.strength ? `• ${med.strength}` : ""} {med.price ? `• ${Number(med.price).toLocaleString()} ${med.price_currency || "EGP"}` : ""}</div><div className="text-xs text-muted-foreground">{med.barcode || "No barcode"} {med.code ? `• ${med.code}` : ""}</div></button>)}</div>}</div>

        <div className="space-y-3">{medicines.map((line, index) => <div key={index} className="grid gap-2 rounded-lg border p-3 md:grid-cols-[1fr_90px_1fr_auto]"><div><Input value={line.name_en} placeholder="Medicine name" onChange={e => updateLine(index, { name_en: e.target.value, medicine_id: null, catalog_product_id: null })} />{line.catalog_product_id && <div className="mt-1 text-xs text-muted-foreground">Catalog product #{line.catalog_product_id}{line.medicine_id ? ` · legacy link #${line.medicine_id}` : " · no legacy link"}</div>}</div><Input type="number" min={1} value={line.quantity} onChange={e => updateLine(index, { quantity: Number(e.target.value) })} /><Input value={line.notes} placeholder="Notes / dose" onChange={e => updateLine(index, { notes: e.target.value })} /><Button type="button" variant="ghost" size="icon" onClick={() => removeLine(index)}><Trash2 className="h-4 w-4" /></Button></div>)}</div>
