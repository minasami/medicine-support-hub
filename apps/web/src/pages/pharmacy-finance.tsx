 ...
 async function load(){
  ...
  try{
   ...
   const [entryRows,summaryRows]=await Promise.all([
    supabaseFetch<Entry[]>(`/rest/v1/pharmacy_finance_entries?select=id,entry_date,entry_type,category,amount&branch_id=eq.${active}&order=entry_date.desc,created_at.desc&limit=50`),
