...
export default function PharmacyFinance() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : "Could not load finance workspace.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function createBranch() {
    ...
    try {
      ...
      const existing = branches.find((branch) => normalize(branch.branch_name) === normalize(name) && normalize(branch.city) === normalize(cityValue));
      if (existing) {
        setBranchId(existing.id);
        setMessage("Branch already exists. I selected it instead of creating a duplicate.");
        return;
      }
      const created = await supabaseFetch<Branch[]>("/rest/v1/pharmacy_branches?select=id,branch_name,city,currency,owner_user_id", {
        method: "POST",
        headers: { Prefer: "return=representation" },
        body: JSON.stringify({ owner_user_id: userId, branch_name: name, city: cityValue || null }),
      });
      const branch = created[0];
      await supabaseFetch("/rest/v1/pharmacy_branch_members", {
        method: "POST",
        body: JSON.stringify({ branch_id: branch.id, user_id: userId, member_role: "owner" }),
      });
      setBranchName("");
      setCity("");
  ...
  async function addEntry() {
    ...
    try {
      await supabaseFetch("/rest/v1/pharmacy_finance_entries", {
        method: "POST",
        body: JSON.stringify({ branch_id: branchId, entry_type: kind, category, amount: Number(amount), entry_date: entryDate, created_by: userId }),
      });
      setAmount("");
      setMessage("Entry added. Profit recalculated.");
      await load();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Could not add entry.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return <div className="container mx-auto max-w-5xl px-4 py-8">
    ...
    <Card className="mb-6"><CardHeader><CardTitle>Report period</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-[1fr_1fr_auto]"><Input type="date" value={fromDate} onChange={event => setFromDate(event.target.value)} /><Input type="date" value={toDate} onChange={event => setToDate(event.target.value)} /><Button variant="outline" onClick={() => void load()}>Apply</Button></CardContent></Card>
