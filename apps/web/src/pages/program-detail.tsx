...
import { AlertCircle, ArrowLeft, CalendarDays, CircleDollarSign, Flag, Plus, RefreshCw, Target, Users } from "lucide-react";
  ...
  return <div className="container mx-auto max-w-7xl px-4 py-8">
    ...
    {program && <>
      <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card><CardContent className="p-5"><div className="flex items-center gap-2 text-sm text-muted-foreground"><CircleDollarSign className="h-4 w-4" />Budget</div><div className="mt-2 text-2xl font-bold">{budget.toLocaleString()} {program.currency}</div></CardContent></Card>
        <Card><CardContent className="p-5"><div className="flex items-center gap-2 text-sm text-muted-foreground"><Flag className="h-4 w-4" />Remaining</div><div className="mt-2 text-2xl font-bold text-emerald-700">{remaining.toLocaleString()} {program.currency}</div></CardContent></Card>
        <Card><CardContent className="p-5"><div className="flex items-center gap-2 text-sm text-muted-foreground"><Users className="h-4 w-4" />Beneficiaries</div><div className="mt-2 text-2xl font-bold">{beneficiaries.length}</div><div className="text-xs text-muted-foreground">Target {program.target_beneficiaries || "not set"} • {coverage}% coverage</div></CardContent></Card>
        <Card><CardContent className="p-5"><div className="flex items-center gap-2 text-sm text-muted-foreground"><Target className="h-4 w-4" />Budget utilization</div><div className="mt-2 text-2xl font-bold">{utilization}%</div><div className="mt-2 h-2 rounded-full bg-slate-100"><div className="h-2 rounded-full bg-blue-600" style={{width:`${utilization}%`}} /></div></CardContent></Card>
      </div>
      <div className="mb-6 grid gap-6 lg:grid-cols-3">
        <Card><CardHeader><CardTitle>Objectives</CardTitle></CardHeader><CardContent><p className="text-sm leading-6 text-muted-foreground">{program.objectives || "No objectives recorded yet."}</p></CardContent></Card>
        <Card><CardHeader><CardTitle>KPI summary</CardTitle></CardHeader><CardContent><p className="text-sm leading-6 text-muted-foreground">{program.kpi_summary || "No KPI summary recorded yet."}</p></CardContent></Card>
        <Card><CardHeader><CardTitle>Timeline</CardTitle></CardHeader><CardContent><div className="space-y-2 text-sm"><div className="flex items-center gap-2"><CalendarDays className="h-4 w-4" />Start: {program.start_date || "Not set"}</div><div className="flex items-center gap-2"><CalendarDays className="h-4 w-4" />End: {program.end_date || "Not set"}</div><div>Status: <Badge variant="secondary">{program.status}</Badge></div></div></CardContent></Card>
      </div>
      <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
        <Card><CardHeader><CardTitle>Add program event</CardTitle></CardHeader><CardContent className="space-y-4"><div><Label>Type</Label><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={draft.event_type} onChange={e=>setDraft({...draft,event_type:e.target.value})}>{["updated","budget","milestone","partner","risk","note"].map(t=><option key={t} value={t}>{t}</option>)}</select></div><div><Label>Title</Label><Input value={draft.title} onChange={e=>setDraft({...draft,title:e.target.value})} /></div><div><Label>Description</Label><Textarea value={draft.description} onChange={e=>setDraft({...draft,description:e.target.value})} /></div><Button onClick={addEvent} disabled={saving || !draft.title.trim()}><Plus className="mr-2 h-4 w-4" />Add event</Button></CardContent></Card>
        <Card><CardHeader><CardTitle>Program activity</CardTitle></CardHeader><CardContent className="space-y-3">{events.length===0?<p className="text-sm text-muted-foreground">No program events yet.</p>:events.map(e=><div key={e.id} className="rounded-lg border p-4"><div className="flex items-center justify-between gap-2"><div className="font-semibold">{e.title}</div><Badge variant="outline">{e.event_type}</Badge></div>{e.description&&<p className="mt-1 text-sm text-muted-foreground">{e.description}</p>}<time className="mt-2 block text-xs text-muted-foreground">{new Date(e.event_date).toLocaleString()}</time></div>)}</CardContent></Card>
      </div>
