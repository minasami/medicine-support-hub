...
function RequestCard({ request, language, note = "", setNote, saving = false, onApprove, onReject, canAct = false }: { request: MedicineRequest; language: string; note?: string; setNote?: (value: string) => void; saving?: boolean; onApprove?: () => void; onReject?: () => void; canAct?: boolean }) {
  ...
  return <Card className={`border-l-4 ${urgent ? "border-l-red-500" : "border-l-blue-500"}`}><CardContent className="space-y-4 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><span className="font-bold">{t("Request", "طلب")} {request.id.slice(0, 8)}</span><Badge variant={urgent ? "destructive" : "secondary"}>{request.priority || "normal"}</Badge></div><div className="mt-1 font-medium">{request.patient_name || t("Patient", "المريض")}</div><div className="text-xs text-muted-foreground">{request.phone || t("No phone recorded", "لا يوجد هاتف مسجّل")}</div></div><div className="flex items-center gap-1 text-xs text-muted-foreground"><Clock className="h-3.5 w-3.5"/>{safeDate(request.created_at)}</div></div>
    <div><div className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{t("Requested medicines", "الأدوية المطلوبة")}</div>{medicines.length ? <div className="space-y-2">{medicines.map((medicine, index) => { const name = language === "ar" ? medicine.name_ar || medicine.name_en : medicine.name_en || medicine.name_ar; return <div key={`${medicine.catalog_product_id || medicine.medicine_id || index}`} className="flex items-start justify-between gap-3 rounded-lg bg-blue-50 px-3 py-2 text-sm"><div><a href={medicine.catalog_product_id ? `/catalog/${medicine.catalog_product_id}` : "/medicines"} className="font-semibold text-primary hover:underline">{name || `Medicine ${index + 1}`}</a>{medicine.notes && <div className="mt-1 text-xs text-muted-foreground">{medicine.notes}</div>}</div><span className="text-xs text-muted-foreground">×{medicine.quantity ?? 1}</span></div>;})}</div> : <div className="rounded-lg border border-dashed p-3 text-sm text-muted-foreground">{t("No structured medicine lines were stored for this request.", "لم تُخزَّن بنود أدوية منظمة لهذا الطلب.")}</div>}</div>
    {request.notes && <div className="rounded-lg bg-muted/40 p-3 text-sm whitespace-pre-wrap">{request.notes}</div>}
    {canAct && <><Textarea value={note} onChange={event => setNote?.(event.target.value)} placeholder={t("Add an accountable review note. A note is required for rejection.", "أضف ملاحظة مراجعة مسؤولة. الملاحظة مطلوبة عند الرفض.")}/><div className="flex flex-wrap gap-2"><Button onClick={onApprove} disabled={saving}><CheckCircle className="mr-2 h-4 w-4"/>{t("Approve request", "الموافقة على الطلب")}</Button><Button variant="destructive" onClick={onReject} disabled={saving || !note.trim()}><Flag className="mr-2 h-4 w-4"/>{t("Reject with note", "رفض مع ملاحظة")}</Button></div></>}
    {urgent && <div className="flex items-center gap-2 text-xs font-medium text-red-700"><AlertTriangle className="h-4 w-4"/>{t("Priority requires timely human review.", "الأولوية تتطلب مراجعة بشرية في الوقت المناسب.")}</div>}
  </CardContent></Card>;
}
...
function safeDate(value: string) { const date = new Date(value); return Number.isNaN(date.getTime()) ? "—" : date.toLocaleDateString(); }
