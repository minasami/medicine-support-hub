  ...
  return (
    <main className="container mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <Badge>{humanize(patient.identity_verification_status, t)}</Badge>
              <Badge variant="outline">{humanize(patient.status, t)}</Badge>
      ...
      <section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <Metric icon={Stethoscope} label={t("Encounters", "الزيارات")} value={encounters.length} />
        <Metric icon={Pill} label={t("Prescriptions", "الوصفات")} value={prescriptions.length} />
        <Metric icon={FlaskConical} label={t("Orders", "الطلبات")} value={orders.length} />
        <Metric icon={FileHeart} label={t("Results", "النتائج")} value={results.length} />
        <Metric icon={Activity} label={t("Journey events", "أحداث الرحلة")} value={timeline.length} />
      </section>

      <section className="mt-9">
        <h2 className="text-2xl font-bold">{t("Clinical encounters", "الزيارات السريرية")}</h2>
        <div className="mt-4 grid gap-4 xl:grid-cols-2">
          {encounters.map((row) => (
            <Card key={row.id}>
              <CardHeader>
                <div className="flex justify-between gap-3">
                  <CardTitle>{humanize(row.encounter_type, t)}</CardTitle>
                  <Badge>{humanize(row.status, t)}</Badge>
                </div>
                <p className="text-xs text-muted-foreground">{new Date(row.started_at).toLocaleString()}</p>
              </CardHeader>
              <CardContent className="space-y-3 text-sm">
                <Info label={t("Chief complaint", "الشكوى الرئيسية")} value={row.chief_complaint} />
                <Info label={t("Clinical summary", "الملخص السريري")} value={row.clinical_summary} />
                <Info label={t("Diagnosis summary", "ملخص التشخيص")} value={row.diagnosis_summary} />
                <Info label={t("Care plan", "خطة الرعاية")} value={row.care_plan} />
              </CardContent>
            </Card>
          ))}
          {!encounters.length && (
            <Empty text={t("No encounter is visible in this consent scope.", "لا توجد زيارة ظاهرة في نطاق الموافقة.")} />
          )}
        </div>
      </section>

      <section className="mt-9">
        <h2 className="text-2xl font-bold">{t("Medicines and prescriptions", "الأدوية والوصفات")}</h2>
        <div className="mt-4 grid gap-4 xl:grid-cols-2">
            ...
            <Card key={prescription.id}>
              <CardHeader>
                <div className="flex justify-between gap-3">
                  ...
                  <Badge>{humanize(prescription.status, t)}</Badge>
                </div>
                <p className="text-xs text-muted-foreground">{new Date(prescription.authored_at).toLocaleString()}</p>
              </CardHeader>
              <CardContent className="space-y-3">
                <Info label={t("Indication", "دواعي الاستعمال")} value={prescription.clinical_indication} />
                <Info label={t("Instructions", "التعليمات")} value={prescription.instructions} />
                {items
                        ...
                        <div>
                          <div className="font-semibold">
                            {item.canonical_medicine_id ? (
                              <Link href={`/catalog/${item.canonical_medicine_id}`} className="text-primary">
                                {item.medicine_name}
                              </Link>
                            ) : (
                              item.medicine_name
                          ...
                          <div className="mt-1 text-sm text-muted-foreground">
                            {[item.strength, item.dosage_form, item.route].filter(Boolean).join(" · ")}
                          </div>
                        </div>
                        <Badge variant="outline">{humanize(item.dispense_status, t)}</Badge>
                      </div>
          ...
          {!prescriptions.length && (
            <Empty text={t("No prescription is visible in this consent scope.", "لا توجد وصفة ظاهرة في نطاق الموافقة.")} />
          )}
        </div>
      </section>

      <section className="mt-9">
        <h2 className="text-2xl font-bold">{t("Diagnostics and results", "الفحوص والنتائج")}</h2>
        <div className="mt-4 grid gap-4 xl:grid-cols-2">
          {orders.map((order) => (
            <Card key={order.id}>
              <CardHeader>
                <div className="flex justify-between gap-3">
                  ...
                  <div className="flex gap-2">
                    <Badge variant="outline">{humanize(order.service_type, t)}</Badge>
                    <Badge>{humanize(order.status, t)}</Badge>
                  </div>
                </div>
                <p className="text-xs text-muted-foreground">
                  {humanize(order.priority, t)} · {new Date(order.authored_at).toLocaleString()}
                </p>
              </CardHeader>
              <CardContent className="space-y-3 text-sm">
                <Info label={t("Clinical question", "السؤال السريري")} value={order.clinical_question} />
                {results
                  .filter((result) => result.service_order_id === order.id)
                  .map((result) => (
                    <div key={result.id} className="rounded-xl border p-3">
                      <div className="flex justify-between gap-2">
                        <div className="font-semibold">{result.title}</div>
                        <Badge variant="outline">{humanize(result.status, t)}</Badge>
                      </div>
                      {result.summary && <p className="mt-2 text-muted-foreground">{result.summary}</p>}
                      {result.conclusion && <p className="mt-2">{result.conclusion}</p>}
                    </div>
                  ))}
              </CardContent>
            </Card>
          ...
          {!orders.length && (
            <Empty text={t("No diagnostic order is visible in this consent scope.", "لا يوجد طلب تشخيصي ظاهر في نطاق الموافقة.")} />
          )}
        </div>
      </section>

      <section className="mt-9">
        <h2 className="text-2xl font-bold">{t("Care journey timeline", "الجدول الزمني لرحلة الرعاية")}</h2>
          ...
          {timeline.map((event) => (
            <Card key={event.id}>
              <CardContent className="grid gap-3 p-4 md:grid-cols-[190px_1fr_auto] md:items-center">
                ...
                <Badge variant="outline">
                  {event.status ? humanize(event.status, t) : humanize(event.event_type, t)}
                </Badge>
              </CardContent>
            </Card>
          ))}
          {!timeline.length && (
            <Empty text={t("No journey event is visible yet.", "لا توجد أحداث ظاهرة بعد.")} />
          )}
        </div>
      </section>
    </main>
...
const STATUS_AR: Record<string, string> = {
  ...
  signed: "موقّع",
  dispensed: "تم الصرف",
  partially_dispensed: "صُرف جزئيًا",
  not_dispensed: "لم يُصرف",
...
function humanize(value: string, t: (en: string, ar: string) => string) {
  const key = value.trim().toLowerCase();
  const en = value
    .replaceAll("_", " ")
    .replace(/\b\w/g, (letter) => letter.toUpperCase());
  const ar = STATUS_AR[key] || STATUS_AR[key.replace(/\s+/g, "_")] || en;
  return t(en, ar);
}
