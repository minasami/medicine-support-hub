...
interface Campaign { id: string; title: string; body: string; audience_type: string; audience_values: string[]; notification_topic: string; target_url: string; status: string; attempted_count: number; delivered_count: number; failed_count: number; created_at: string; completed_at: string | null }
interface Subscription { id: string; user_id: string | null; topics: string[]; is_enabled: boolean; last_seen_at: string }
interface CatalogMetrics { canonical_medicines: number; company_profiles: number; active_push_subscriptions: number; notification_campaigns: number }
...
export default function AdminNotifications() {
  ...
  const [metrics, setMetrics] = useState<CatalogMetrics>({ canonical_medicines: 0, company_profiles: 0, active_push_subscriptions: 0, notification_campaigns: 0 });
  ...
  const isAdmin = Boolean(me?.is_active && ["admin", "platform_admin", "super_admin"].includes(me.role));
  ...
  async function load() {
    ...
    try {
      ...
      const profiles = await supabaseFetch<Profile[]>(`/rest/v1/profiles?select=id,role,is_active&id=eq.${session.user.id}&limit=1`);
      ...
      const [nextCampaigns, nextSubscriptions, metricRows] = await Promise.all([
        ...
        supabaseFetch<CatalogMetrics[]>("/rest/v1/rpc/platform_catalog_notification_metrics", { method: "POST", body: "{}" }),
      ]);
      setCampaigns(nextCampaigns || []);
      setSubscriptions(nextSubscriptions || []);
      setMetrics(metricRows[0] || { canonical_medicines: 0, company_profiles: 0, active_push_subscriptions: 0, notification_campaigns: 0 });
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not load notification management."); }
  ...
  function applyTemplate(template: typeof templates[number]) {
    const enrichedBody = template.topic === "medicine_updates"
      ? `${template.body} The live encyclopedia currently connects ${Number(metrics.canonical_medicines).toLocaleString()} canonical records.`
      : template.topic === "company_updates"
        ? `${template.body} The company directory currently contains ${Number(metrics.company_profiles).toLocaleString()} manufacturer-derived profiles.`
        : template.body;
    setDraft((current) => ({ ...current, title: template.title, body: enrichedBody.slice(0, 500), topic: template.topic, targetUrl: template.target, audienceType: "topic", audienceValues: "" }));
  }
  ...
  async function sendCampaign(event: React.FormEvent) {
    ...
    try {
      ...
      setMessage(`Campaign completed: ${Number(result.delivered || 0).toLocaleString()} delivered, ${Number(result.failed || 0).toLocaleString()} failed, ${Number(result.attempted || 0).toLocaleString()} attempted.`);
      setDraft({ title: "", body: "", topic: "platform_updates", targetUrl: "/", audienceType: "topic", audienceValues: "", imageUrl: "" });
      await load();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not send the notification campaign."); }
  ...
  return <main className="container mx-auto max-w-7xl px-4 py-8">
    ...
    {!loading && isAdmin && <>
      <section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Active push subscriptions" value={Number(metrics.active_push_subscriptions || subscriptions.filter((row) => row.is_enabled).length)} /><Metric label="Campaigns" value={Number(metrics.notification_campaigns || campaigns.length)} /><Metric label="Aggregate delivery rate" value={`${deliveryRate}%`} /><Metric label="Available topics" value={topics.length} /></section>
