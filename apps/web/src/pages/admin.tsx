...
import { AlertCircle, Building2, Database, RefreshCw, ShieldCheck, Users } from "lucide-react";
...
async function api<T>(path: string, session: Session, init: RequestInit = {}) {
  ...
  const res = await fetch(`${url}${path}`, { ...init, headers: { apikey: key, Authorization: `Bearer ${session.access_token}`, "Content-Type": "application/json", Accept: "application/json", ...(init.headers ?? {}) } });
...
export default function AdminPortal() {
  ...
  const stats = useMemo(() => ({ users: users.length, orgs: orgs.length, requests: requests.length, pending: requests.filter(r => r.status === "pending").length }), [users, orgs, requests]);
  ...
  async function load() {
    ...
    try {
      ...
      try { setOrgs(await api<Org[]>(`/rest/v1/organizations?select=${ORG_SELECT}&order=name.asc&limit=500`, s)); }
      catch { setOrgs([]); setMessage("Organizations table is not ready yet. Apply the platform foundation migration and RLS policies."); }
  ...
  async function updateUser(user: Profile, patch: Partial<Profile>) {
    ...
    try {
      ...
      setUsers(cur => cur.map(u => u.id === user.id ? { ...u, ...(updated[0] ?? patch) } : u));
  ...
  return <div className="container mx-auto max-w-7xl px-4 py-8">
    ...
    {!loading && isAdmin && <>
      <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Card><CardContent className="p-5"><div className="text-3xl font-bold">{stats.orgs}</div><div className="text-sm text-muted-foreground">Organizations</div></CardContent></Card><Card><CardContent className="p-5"><div className="text-3xl font-bold">{stats.users}</div><div className="text-sm text-muted-foreground">Users</div></CardContent></Card><Card><CardContent className="p-5"><div className="text-3xl font-bold">{stats.requests}</div><div className="text-sm text-muted-foreground">Requests</div></CardContent></Card><Card><CardContent className="p-5"><div className="text-3xl font-bold text-amber-600">{stats.pending}</div><div className="text-sm text-muted-foreground">Pending</div></CardContent></Card></div>
