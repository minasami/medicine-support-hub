...
function WhatsAppChoice({ same, number, onSameChange, onNumberChange, t }: { same: boolean; number: string; onSameChange: (value: boolean) => void; onNumberChange: (value: string) => void; t: (en: string, ar: string) => string }) {
  return <div className="rounded-xl border p-4"><div className="flex items-center gap-2 font-semibold"><Phone className="h-4 w-4 text-primary" />{t("Is the mobile number also used for WhatsApp?", "هل رقم الهاتف المحمول هو نفسه رقم واتساب؟")}</div><div className="mt-3 flex flex-wrap gap-3"><label className={`flex min-h-11 cursor-pointer items-center gap-2 rounded-lg border px-4 py-2 text-sm ${same ? "border-primary bg-primary/5" : ""}`}><input type="radio" name={`whatsapp-${number}`} checked={same} onChange={() => onSameChange(true)} />{t("Yes, use the mobile number", "نعم، استخدم رقم الهاتف")}</label><label className={`flex min-h-11 cursor-pointer items-center gap-2 rounded-lg border px-4 py-2 text-sm ${!same ? "border-primary bg-primary/5" : ""}`}><input type="radio" name={`whatsapp-${number}`} checked={!same} onChange={() => onSameChange(false)} />{t("No, WhatsApp is different", "لا، رقم واتساب مختلف")}</label></div>{!same && <div className="mt-4"><Field label={t("WhatsApp number", "رقم واتساب")} type="tel" value={number} onChange={onNumberChange} placeholder="+20 10 0000 0000" required /></div>}</div>;
}
