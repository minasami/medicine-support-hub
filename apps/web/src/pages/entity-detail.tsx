...
export default function EntityDetail() {
  ...
  async function loadCompanyProducts(companySlug: string, search: string, offset = 0, append = false) {
    ...
    try {
      const rows = await supabaseFetch<Product[]>(`/rest/v1/rpc/company_medicine_portfolio_page?p_company_slug=${encode(companySlug)}&p_query=${encode(search.trim())}&p_limit=${PAGE_SIZE}&p_offset=${offset}`);
    ...
    async function load() {
      ...
      try {
        ...
        if (type === "company") {
          ...
          const [sourceRows, officialRows, contributionRows] = await Promise.all([supabaseFetch<CompanyProfile[]>(`/rest/v1/medicine_company_profiles?select=${sourceSelect}&company_slug=eq.${encode(normalizedSlug)}&limit=1`), supabaseFetch<OfficialProfile[]>(`/rest/v1/industry_company_profiles?select=${officialSelect}&company_slug=eq.${encode(normalizedSlug)}&verification_status=eq.verified&is_public=eq.true&limit=1`), supabaseFetch<CompanyContribution[]>(`/rest/v1/industry_company_contributions?select=id,contribution_type,title,summary,evidence_urls,published_at&company_slug=eq.${encode(normalizedSlug)}&status=eq.approved&published_at=not.is.null&order=published_at.desc&limit=50`)]);
  ...
  const description = entity ? officialProfile?.description || (type === "company" ? `${entity.name} company intelligence profile connecting ${Number(activeCount).toLocaleString()} active source-backed medicine records, ${Number(genericCount).toLocaleString()} generics, and ${Number(diseaseCount).toLocaleString()} therapeutic areas.` : `${entity.name} medicine reference connecting ${entity.records.toLocaleString()} active source-backed products, companies, generics, disease areas, prescription signals, and observed source-market prices.`) : "Source-backed medicine entity page.";
  ...
  return (
    <main className="container mx-auto max-w-7xl px-4 py-8">
      <a href={directoryPath} className="inline-flex items-center text-sm font-semibold text-primary">
      ...
      {entity && !loading && (
        <>
            ...
            <div className="flex flex-col gap-5 md:flex-row md:items-start">
              ...
              <div className="min-w-0 flex-1">
                <p className="flex items-center gap-2 text-sm font-medium uppercase tracking-wide text-muted-foreground">
                  ...
                  {type === "company" ? t("Healthcare company and medicine portfolio", "شركة رعاية صحية ومحفظة أدوية") : t("Medicine evidence reference", "مرجع أدلة دوائية")}
                </p>
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  <h1 className="text-3xl font-bold tracking-tight">{officialProfile?.display_name || entity.name}</h1>
                  {officialProfile && (
                ...
                {type === "company" && cleanCompanyOrigin(companyProfile?.origin || entity.origin || officialProfile?.country) && (
                  <p className="mt-2 text-muted-foreground">
                    {t("Origin or headquarters", "المنشأ أو المقر")}: {cleanCompanyOrigin(companyProfile?.origin || entity.origin || officialProfile?.country)}
                  </p>
                )}
          ...
          <div className="mt-4">
            <ShareContributeActions title={officialProfile?.display_name || entity.name} contributionUrl={type === "company" ? `/industry?company=${encode(entity.slug)}#participate` : `/industry?entity=${encode(entity.name)}#participate`} />
          </div>
          ...
          {type === "company" && !officialProfile && (
            <section className="mt-6 rounded-2xl border border-dashed p-5">
              <h2 className="text-lg font-semibold">{t("Represent this company?", "هل تمثل هذه الشركة؟")}</h2>
              <p className="mt-2 text-sm text-muted-foreground">{t("Submit a profile claim. Automated checks score work-email, website-domain, dataset match, and evidence signals; final ownership still requires platform-admin approval.", "أرسل طلب المطالبة بالملف. تفحص الأتمتة بريد العمل ونطاق الموقع ومطابقة قاعدة البيانات وإشارات الأدلة، بينما تظل الموافقة النهائية بيد مسؤول المنصة.")}</p>
              <a href={`/industry?company=${encode(entity.slug)}#participate`} className="mt-4 inline-flex rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground">
          ...
          {type === "generic" && (
            <div className="mt-5">
              <Button asChild>
                <a href={`/medicines?scientific=${encodeURIComponent(entity.name)}`}>{t("Browse canonical medicines with this active ingredient", "تصفح الأدوية الموحدة بهذه المادة الفعالة")}</a>
              </Button>
            </div>
          )}
          ...
          {type !== "company" && (
            <section className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <Metric label={t("Companies", "الشركات")} value={new Set(products.map((product) => product.company_name).filter(Boolean)).size} />
              <Metric label={type === "disease" ? t("Generics", "المواد الفعالة") : t("Disease areas", "المجالات المرضية")} value={type === "disease" ? new Set(products.map((product) => product.generic_name).filter(Boolean)).size : new Set(products.map((product) => product.disease_name).filter(Boolean)).size} />
            ...
            <section className="mt-6 rounded-2xl border bg-card p-5 shadow-sm">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {related.slice(0, 48).map((item) => (
                  <a key={`${item.type}-${item.slug}`} href={seoEntityPath(item.type, item.slug)} className="rounded-full border px-3 py-1.5 text-sm font-medium hover:border-primary/50 hover:bg-muted">
                    {item.name}
                  </a>
                ))}
              </div>
          ...
          {type === "company" && contributions.length > 0 && (
            <section className="mt-6">
              <h2 className="text-2xl font-semibold">{t("Approved company contributions", "مساهمات الشركة المعتمدة")}</h2>
                  ...
                  <Card key={contribution.id}>
                    <CardHeader>
                      <div className="flex flex-wrap items-start justify-between gap-2">
                        ...
                        <Badge variant="outline">{humanize(contribution.contribution_type)}</Badge>
                      </div>
                    </CardHeader>
                    <CardContent>
                      <p className="text-sm leading-6 text-muted-foreground">{contribution.summary}</p>
                      {contribution.evidence_urls.length > 0 && (
                        <div className="mt-4 flex flex-col gap-1">
                          {contribution.evidence_urls.slice(0, 5).map((url) => (
                            <a key={url} href={url} target="_blank" rel="noreferrer" className="inline-flex items-center break-all text-sm font-semibold text-primary">
                              {t("Open supporting evidence", "فتح الدليل الداعم")}
          ...
          <section className="mt-6">
            <div className="flex flex-wrap items-end justify-between gap-3">
              <div>
                <h2 className="text-2xl font-semibold">{type === "company" ? t("Company medicine portfolio", "محفظة أدوية الشركة") : t("Verified source products", "منتجات مصدرية موثقة")}</h2>
                <p className="mt-1 text-sm text-muted-foreground">{type === "company" ? t("Every medicine below links back to its canonical encyclopedia page.", "كل دواء أدناه يرتبط بصفحته في الموسوعة الموحدة.") : t("Independent source records remain separate from official company information.", "تظل سجلات المصادر المستقلة منفصلة عن معلومات الشركة الرسمية.")}</p>
              </div>
            </div>
            {type === "company" && (
              <form
              ...
              >
                <Input value={portfolioQuery} onChange={(event) => setPortfolioQuery(event.target.value)} placeholder={t("Search this portfolio by product, generic, therapy, or form...", "ابحث داخل المحفظة بالمنتج أو المادة الفعالة أو المجال أو الشكل...")} />
            ...
            {type === "company" && (
              <p className="mt-3 text-sm text-muted-foreground">
                {portfolioTotal.toLocaleString()} {t("matching canonical medicine records", "سجل دواء موحد مطابق")}
              </p>
            )}
            {products.length > 0 ? (
            ...
            {type === "company" && products.length < portfolioTotal && (
              <div className="mt-5 flex justify-center">
                <Button variant="outline" disabled={loadingProducts} onClick={() => void loadCompanyProducts(entity.slug, portfolioQuery, products.length, true)}>
                  {loadingProducts && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                  {t("Load more portfolio medicines", "تحميل المزيد من أدوية المحفظة")}
                </Button>
              </div>
            )}
          </section>
  ...
  return (
    <section className="mt-6 rounded-2xl border bg-card p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold">{t("Encyclopedia-derived company intelligence", "معلومات الشركة المشتقة من الموسوعة")}</h2>
          <p className="mt-1 text-sm text-muted-foreground">{t("Computed from canonical medicine-company relationships. Toll manufacturers and trademark owners remain separate entities and are kept separate from official company statements.", "محسوبة من علاقات الأدوية بالشركات في الموسوعة الموحدة. يظل المصنعون لحساب الغير ومالكو العلامات التجارية كيانات منفصلة، كما تظل هذه البيانات منفصلة عن بيانات الشركة الرسمية.")}</p>
        </div>
        <Badge variant={imported ? "secondary" : "outline"}>{imported ? t("Canonical portfolio loaded", "المحفظة الموحدة محملة") : t("Aggregate portfolio ready", "ملخص المحفظة جاهز")}</Badge>
      </div>
      {roles.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-2">
          {roles.map((role) => (
            <Badge key={role} variant={role === "trademark_owner" ? "secondary" : "outline"}>
              {medicineCompanyRoleLabel(role, t)} · {companyRelationshipCount(profile, role).toLocaleString()} {t("medicines", "دواء")}
            </Badge>
          ))}
        </div>
      )}
      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Metric label={t("Portfolio medicines", "أدوية المحفظة")} value={profile.active_product_count} />
        <Metric label={t("Generics", "المواد الفعالة")} value={profile.generic_count} />
        <Metric label={t("Therapeutic categories", "الفئات العلاجية")} value={profile.disease_area_count} />
        <Metric label={t("Observed price range", "نطاق السعر المرصود")} value={profile.min_price != null && profile.max_price != null ? `${Number(profile.min_price).toLocaleString()}–${Number(profile.max_price).toLocaleString()} ${profile.source_currency}` : "—"} />
      </div>
      <div className="mt-5 grid gap-5 lg:grid-cols-3">
        <TagGroup title={t("Leading therapeutic categories", "أبرز الفئات العلاجية")} values={list(profile.therapeutic_areas)} hrefFor={(value) => seoEntityPath("disease", seoEntitySlug(cleanDiseaseEntityName(value)))} />
        <TagGroup title={t("Leading generics", "أبرز المواد الفعالة")} values={list(profile.leading_generics)} hrefFor={(value) => seoEntityPath("generic", seoEntitySlug(value))} />
        <TagGroup title={t("Portfolio sample", "عينة من المحفظة")} values={list(profile.portfolio_sample)} hrefFor={(value) => `/medicines?q=${encode(value)}`} />
      </div>
      <p className="mt-4 text-xs text-muted-foreground">
        {t("Source", "المصدر")}: {profile.source_name} · {t("Currency", "العملة")}: {profile.source_currency}
        {profile.latest_source_update ? ` · ${t("Refreshed", "آخر تحديث")}: ${new Date(profile.latest_source_update).toLocaleDateString()}` : ""}
      </p>
    </section>
  );
}
  ...
  return (
    <section className="mt-6 rounded-2xl border border-primary/25 bg-primary/5 p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {humanize(profile.company_type)} · {[profile.city, profile.country].filter(Boolean).join(", ")}
          </p>
        </div>
        {profile.website_url && (
          <a href={profile.website_url} target="_blank" rel="noreferrer" className="inline-flex items-center rounded-lg border bg-background px-4 py-2 text-sm font-semibold">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <TagGroup title={t("Therapeutic areas", "المجالات العلاجية")} values={profile.therapeutic_areas} hrefFor={(value) => seoEntityPath("disease", seoEntitySlug(cleanDiseaseEntityName(value)))} />
        <TagGroup title={t("Product categories", "فئات المنتجات")} values={profile.product_categories} hrefFor={(value) => `/search?q=${encode(value)}`} />
        <TagGroup title={t("Capabilities", "القدرات")} values={profile.capabilities} hrefFor={(value) => `/search?q=${encode(value)}`} />
        <TagGroup title={t("Patient-support programs", "برامج دعم المرضى")} values={profile.support_programs} hrefFor={(value) => `/search?q=${encode(value)}`} />
      </div>
    </section>
  );
}
    ...
    <Card className="mt-4">
      <CardContent className="p-6 text-sm text-muted-foreground">
        {type === "company" && sample.length > 0 ? (
          <>
            <p>{t("The aggregate portfolio is available while detailed canonical links are being resolved.", "ملخص المحفظة متاح بينما يتم استكمال ربط السجلات الموحدة التفصيلية.")}</p>
          ...
          t("No independent source products are linked yet.", "لا توجد منتجات مرتبطة بمصدر مستقل حتى الآن.")
        )}
      </CardContent>
    </Card>
  );
...
function TagGroup({ title, values, hrefFor }: { title: string; values: string[]; hrefFor?: (value: string) => string }) {
  return (
    <div>
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {values.length > 0 ? (
          values.slice(0, 12).map((value) =>
            hrefFor ? (
              <a key={value} href={hrefFor(value)} className="rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                <Badge variant="secondary" className="cursor-pointer transition hover:bg-primary hover:text-primary-foreground">
                  {value}
                </Badge>
              </a>
            ) : (
...
function ProductCard({ product, t }: { product: Product; t: (en: string, ar: string) => string }) {
  ...
  const medicineLink = product.product_url || `/medicines?q=${encode(product.product_name)}`;
  return (
    <Card className="h-full shadow-sm transition hover:border-primary/40 hover:shadow-md">
      <CardHeader>
        <CardTitle className="text-lg leading-7">
          <a href={medicineLink} target={internal || !product.product_url ? undefined : "_blank"} rel={internal || !product.product_url ? undefined : "noreferrer"} className="hover:text-primary hover:underline">
            {product.product_name}
          </a>
        </CardTitle>
        {product.generic_name ? (
          <a href={seoEntityPath("generic", seoEntitySlug(product.generic_name))} className="text-sm text-muted-foreground hover:text-primary hover:underline">
            {product.generic_name}
          </a>
        ) : (
          <p className="text-sm text-muted-foreground">{product.drug_variant || "—"}</p>
        )}
      </CardHeader>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {product.final_price != null && (
            <Badge variant="secondary">
              {Number(product.final_price).toLocaleString()} {product.price_currency}
            </Badge>
          )}
        </div>
        {product.company_name && product.company_slug && (
          <a href={seoEntityPath("company", product.company_slug)} className="inline-flex items-center font-semibold text-primary hover:underline">
            <Building2 className="mr-1.5 h-4 w-4" />
            {product.company_name}
          </a>
        )}
        {product.drug_content_summary && <p className="line-clamp-3 text-muted-foreground">{product.drug_content_summary}</p>}
        <a href={medicineLink} target={internal || !product.product_url ? undefined : "_blank"} rel={internal || !product.product_url ? undefined : "noreferrer"} className="inline-flex items-center font-semibold text-primary hover:underline">
          {internal || !product.product_url ? t("Open medicine encyclopedia page", "فتح صفحة الدواء في الموسوعة") : t("Open source listing", "فتح قائمة المصدر")}
