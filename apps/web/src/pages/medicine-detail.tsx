...
type Product = {
  canonical_id:number; canonical_key:string; name_en:string|null; name_ar:string|null; scientific_name:string|null;
  manufacturer:string|null; drug_class:string|null; route:string|null; category:string|null; image_url:string|null;
  egyptdwa_source_url:string|null; barcode:string|null; code:string|null; custom_product_code:string|null;
  current_price_egp:number|null; price_currency:string; min_price_egp:number|null; max_price_egp:number|null;
  price_observation_count:number; distinct_price_count:number; has_price_history:boolean; source_record_count:number;
  source_count:number; source_systems:string[]; has_verified_dataset:boolean; has_operational_catalog:boolean;
  has_egyptdwa_source:boolean; has_company_verified_source:boolean; company_product_count:number; company_slugs:string[];
  marketplace_offer_count:number; marketplace_seller_count:number; lowest_marketplace_price_egp:number|null;
  current_price_source:string|null; current_price_observed_at:string|null; current_price_date_precision:string|null;
};
type PriceHistory={price:number;currency:string;source_system:string;source_name:string;first_observed_at:string;last_observed_at:string;date_precision:string;source_record_count:number;current_price_egp:number;is_current_candidate:boolean;price_delta_from_previous:number|null};
type Offer={id:string;canonical_id:number;seller_profile_id:string;seller_slug:string;seller_name:string;seller_type:string;seller_country:string|null;seller_city:string|null;unit_price_egp:number;list_price_egp:number|null;minimum_order_quantity:number;packaging:string|null;stock_status:string;lead_time_days:number|null;minimum_expiry_months:number|null;delivery_scope:string[];advantages:string[];payment_terms:string[];cold_chain_supported:boolean;published_at:string;price_difference_percent:number|null};
type Contribution={id:string;contribution_type:string;title:string;summary:string;proposed_price_egp:number|null;organization_name:string|null;evidence_urls:string[];created_at:string};
...
export default function MedicineDetail(){
  ...
  async function load(){setLoading(true);setError(null);try{
    ...
    const [products,priceRows,offerRows,contributionRows]=await Promise.all([
    ...
    const next=products[0]||null;if(!next)throw new Error(t("Medicine record not found.","لم يتم العثور على سجل الدواء."));
    setProduct(next);setHistory(priceRows);setOffers(offerRows);setContributions(contributionRows);
    if(!catalogRoute&&window.location.pathname!==`/catalog/${canonicalId}`)window.history.replaceState({},"",`/catalog/${canonicalId}`);
  }catch(cause){setError(cause instanceof Error?cause.message:t("Could not load the medicine.","تعذر تحميل الدواء."));}finally{setLoading(false);}}
  ...
  async function submitContribution(event:React.FormEvent){event.preventDefault();if(!session?.user?.id||!product)return;setSaving(true);setError(null);setMessage(null);try{
    await supabaseFetch("/rest/v1/medicine_collaboration_submissions",{method:"POST",headers:{Prefer:"return=representation"},body:JSON.stringify({canonical_id:product.canonical_id,contribution_type:draft.type,title:draft.title.trim(),summary:draft.summary.trim(),proposed_price_egp:draft.price?Number(draft.price):null,evidence_urls:draft.evidence.split(/[\n,]/).map(value=>value.trim()).filter(Boolean),submitted_by:session.user.id,organization_name:draft.organization.trim()||null,status:"submitted"})});
    setDraft({type:"correction",title:"",summary:"",price:"",organization:"",evidence:""});setMessage(t("Contribution submitted for evidence review.","تم إرسال المساهمة لمراجعة الأدلة."));
  }catch(cause){setError(cause instanceof Error?cause.message:t("Could not submit the contribution.","تعذر إرسال المساهمة."));}finally{setSaving(false);}}
  ...
  return <main className="container mx-auto max-w-7xl px-4 py-8">
    ...
    {error&&<Alert variant="destructive" className="mt-5"><AlertCircle className="h-4 w-4"/><AlertDescription>{error}</AlertDescription></Alert>}{message&&<Alert className="mt-5"><Send className="h-4 w-4"/><AlertDescription>{message}</AlertDescription></Alert>}

    <section className="mt-6 overflow-hidden rounded-3xl border bg-card shadow-sm"><div className="grid gap-8 p-6 md:p-10 lg:grid-cols-[.75fr_1.25fr]">{product.image_url?<img src={product.image_url} alt={title} className="max-h-[380px] w-full rounded-2xl bg-muted/30 object-contain p-5"/>:<div className="flex min-h-64 items-center justify-center rounded-2xl bg-muted/30"><Info className="h-12 w-12 text-muted-foreground"/></div>}<div><div className="flex flex-wrap gap-2">{product.has_verified_dataset&&<Badge><ShieldCheck className="mr-1 h-3 w-3"/>{t("Verified dataset","بيانات موثقة")}</Badge>}{product.has_company_verified_source&&<Badge variant="secondary"><Building2 className="mr-1 h-3 w-3"/>{t("Verified company portfolio","محفظة شركة موثقة")}</Badge>}{product.marketplace_offer_count>0&&<Badge variant="outline"><Store className="mr-1 h-3 w-3"/>{product.marketplace_offer_count} {t("approved offers","عرض معتمد")}</Badge>}</div><h1 className="mt-4 text-4xl font-bold tracking-tight">{title}</h1>{alternate&&<p className="mt-2 text-lg text-muted-foreground">{alternate}</p>}<div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-3"><Metric label={t("Evidence price candidate","سعر الدليل المرشح")} value={formatPrice(product.current_price_egp)}/><Metric label={t("Observed price range","نطاق الأسعار المرصودة")} value={product.min_price_egp!=null&&product.max_price_egp!=null?`${Number(product.min_price_egp).toLocaleString()}–${Number(product.max_price_egp).toLocaleString()} EGP`:"—"}/><Metric label={t("Approved supply offers","عروض التوريد المعتمدة")} value={product.marketplace_offer_count}/></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><Fact label={t("Scientific name","الاسم العلمي")} value={product.scientific_name}/><Fact label={t("Manufacturer","الشركة المصنعة")} value={product.manufacturer}/><Fact label={t("Drug class","التصنيف الدوائي")} value={product.drug_class||product.category}/><Fact label={t("Route","طريقة الاستخدام")} value={product.route}/><Fact label={t("Barcode","الباركود")} value={product.barcode}/><Fact label={t("Product code","كود المنتج")} value={product.code||product.custom_product_code}/></div><div className="mt-6 flex flex-wrap gap-2">{product.source_systems.map(source=><Badge variant="outline" key={source}>{humanize(source)}</Badge>)}</div>{product.egyptdwa_source_url&&<a href={product.egyptdwa_source_url} target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center font-semibold text-primary">{t("Open attributed source listing","فتح قائمة المصدر المنسوبة")}<ExternalLink className="ml-2 h-4 w-4"/></a>}</div></div></section>

    <Alert className="mt-6"><AlertDescription>{t("Evidence price and seller offer are different concepts. The evidence price is the highest connected EGP observation under the platform rule. Marketplace prices are reviewed seller offers and still require licensing, quality, expiry, batch, prescription, availability, and procurement verification.","سعر الدليل وعرض البائع مفهومان مختلفان. سعر الدليل هو أعلى قيمة جنيه مرتبطة وفق قاعدة المنصة، أما أسعار السوق فهي عروض بائعين مراجعة وتظل بحاجة للتحقق من الترخيص والجودة والصلاحية والتشغيلة والوصفة والتوافر وضوابط الشراء.")}</AlertDescription></Alert>
