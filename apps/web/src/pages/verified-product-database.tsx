    ...
    <main className="container mx-auto max-w-6xl px-4 py-8">
      <section className="rounded-2xl border bg-card p-6 shadow-sm">
        <p className="flex items-center gap-2 text-sm font-medium uppercase tracking-wide text-muted-foreground">
          ...
          {t("Verified CSV database", "قاعدة CSV موثقة")}
        </p>
      ...
      <div className="mt-6">
        <ConnectedNextActions contextType="module" contextKey="verified-products" />
      </div>

      <section className="mt-6 flex flex-wrap gap-2">
        <a href="/companies" className="rounded-lg border px-4 py-2 text-sm font-semibold hover:bg-muted">
          {t("Company profiles", "ملفات الشركات")}
        </a>
        <a href="/generics" className="rounded-lg border px-4 py-2 text-sm font-semibold hover:bg-muted">
          {t("Generic directory", "دليل المواد الفعالة")}
        </a>
      ...
      <section className="mt-6 rounded-2xl border bg-card p-5 shadow-sm">
        ...
        {companySlug && (
          <div className="mt-3 rounded-lg border bg-muted/40 px-3 py-2 text-sm text-muted-foreground">
            {t("Filtered by company profile", "تمت الفلترة حسب ملف الشركة")}: {companySlug}
          </div>
        )}
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            <Card key={product.id} className="shadow-sm">
              <CardHeader>
                ...
                {product.generic_name ? (
                  <a href={seoEntityPath("generic", seoEntitySlug(product.generic_name))} className="text-sm font-medium text-primary hover:underline">
                    {product.generic_name}
                  </a>
                ) : (
              ...
              <CardContent className="space-y-3 text-sm">
                ...
                <div>
                  <div className="text-xs text-muted-foreground">{t("Company", "الشركة")}</div>
                  {product.company_name && product.company_slug ? (
                    <a href={seoEntityPath("company", product.company_slug)} className="font-medium text-primary hover:underline">
                      {product.company_name}
                    </a>
                  ) : (
                    <div className="font-medium">{product.company_name || "—"}</div>
                  )}
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                  {product.company_slug && (
                    <a href={seoEntityPath("company", product.company_slug)} className="inline-flex items-center font-semibold text-primary">
                      {t("Company profile", "ملف الشركة")}
                    </a>
                  )}
                  {product.product_url && (
                    <a href={product.product_url} target="_blank" rel="noreferrer" className="inline-flex items-center font-semibold text-primary">
                      {t("Source listing", "قائمة المصدر")}
        ...
        {!loading && products.length === 0 && (
          <Card>
            <CardContent className="p-6 text-sm text-muted-foreground">{t("No active products found.", "لا توجد منتجات نشطة مطابقة.")}</CardContent>
          </Card>
        )}
      </section>
    ...
    <label className="text-sm">
      ...
      <select value={value} onChange={(event) => onChange(event.target.value)} className="w-full rounded-md border bg-background px-3 py-2">
        ...
        {rows.slice(0, 30).map((row) => (
          <option key={`${row.facet_type}-${row.facet_value}`} value={row.facet_value}>
            {row.facet_value} ({row.records})
          </option>
        ))}
      </select>
    </label>
